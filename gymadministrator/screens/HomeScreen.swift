    ...
    class AdminMembershipManager: ObservableObject {
        ...
        private func sendEnhancedReactivationNotification(
                ...
                do {
                    ...
                    let formattedEndDate = DateFormatter.membershipFormatter.string(from: endDate)
                    
                    let title = "🎉 ¡Membresía Reactivada!"
                    let body = """
                    ¡Hola \(userName)! Tu membresía ha sido reactivada.
                    
                    ⏰ Días disponibles: \(daysRestored)
                    📅 Vence el: \(formattedEndDate)
                    
                    ¡Ya puedes volver a entrenar! 💪
                    """
                    ...
                    await FCMNotificationManager.shared.sendNotificationToUser(
                        ...
                        data: [
                            ...
                            "days_available": "\(daysRestored)",
                            "end_date": formattedEndDate,
                            "activation_type": "direct_reactivation",
                            "no_charge": "true", // ✅ Indicar que no hubo cobro
                            "timestamp": "\(Date().timeIntervalSince1970)"
                        ],
                        directToken: fcmToken
                    )
                    
                    print("📱 Notificación de reactivación sin cobro enviada")
        ...
        func reactivateMembershipPreservingDays(membershipId: String, userEmail: String) async {
                do {
                    ...
                    try await db.collection("membresias").document(membershipId).updateData([
                        ...
                        "activacionTipo": "directa_preservada" // ✅ Marcar como activación directa
                    ])
                    
                    print("✅ Membresía reactivada con \(preservedDays) días restaurados")
                    
                    // 5. Enviar notificación de reactivación mejorada
                    await sendEnhancedReactivationNotification(
                        userEmail: userEmail,
                        userName: userEmail.components(separatedBy: "@").first ?? "Usuario",
                        daysRestored: preservedDays,
                        endDate: newEndDate
                    )
        ...
        private var pricingSection: some View {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Toggle("Usar precio personalizado", isOn: $useCustomPrice)
                        ...
                        .foregroundColor(.brandLight)
                        .onChange(of: useCustomPrice) { isEnabled in
                            // ✅ LÓGICA CORREGIDA: Si desactiva el toggle, limpiar el campo
                            if !isEnabled {
                                customPrice = ""
                                print("💰 Toggle desactivado - campo de precio limpiado")
                            } else {
                                print("💰 Toggle activado - campo listo para entrada manual")
                            }
                        }
                    ...
                    if useCustomPrice {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            HStack {
                                ...
                                TextField("Ingresa el precio", text: $customPrice) // ✅ PLACEHOLDER MEJORADO
                                    .keyboardType(.numberPad)
                                    .font(.title2)
                                    .fontWeight(.semibold)
                            }
                            ...
                            // ✅ NUEVO: Ayuda visual cuando el campo está vacío
                            if customPrice.isEmpty {
                                Text("💡 Deja vacío para usar precio base: $\(Int(membership.precio).formatted())")
                                    .font(.caption2)
                                    .foregroundColor(.brandLight.opacity(0.6))
                    ...
                    VStack(spacing: 8) {
                        ...
                        // ✅ CORREGIDO: Solo mostrar descuento/recargo si hay precio personalizado válido
                        if useCustomPrice, let customPriceValue = Double(customPrice), customPriceValue > 0 {
                            let discount = membership.precio - customPriceValue
                            if discount > 0 {
                                HStack {
                                    Text("Descuento:")
                            ...
                            // ✅ NUEVO: Mostrar mensaje cuando el toggle está activo pero el campo vacío
                            HStack {
                                Text("Precio:")
                                    .foregroundColor(.brandLight.opacity(0.8))
                                Spacer()
                                Text("Base ($\(Int(membership.precio).formatted()))")
                                    .fontWeight(.semibold)
                                    .foregroundColor(.brandLight.opacity(0.8))
                            }
                        }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(spacing: 12) {
                    ...
                    VStack(spacing: 8) {
                        ...
                        if useCustomPrice && !customPrice.isEmpty && Double(customPrice) != nil {
                                                HStack {
                                                    Text("Tipo de precio:")
                                                    Spacer()
                                                    Text("Personalizado")
